...
float4 paintColor = float4(1,1,1,1);
float reflPower = 1;
...
PSInput VertexShaderFunction(VSInput VS)
{
    ...
    PS.Specular.rgb = gMaterialSpecular.rgb * MTACalculateSpecular( gCameraDirection, gLightDirection, PS.Normal, gMaterialSpecPower ) * specularPower;
    // PS.Specular.rgb = gMaterialSpecular.rgb * MTACalculateSpecular( gCameraDirection*1000000, gLightDirection*5, PS.Normal, gMaterialSpecPower );
...
float4 PixelShaderFunction(PSInput PS) : COLOR0
{
    ...
    float brightnessFactor = 0.3; // the default value is 0.10
    ...
    envMap.rgb = envMap.rgb * envMap.rgb * envMap.rgb * reflPower;
    ...
    float4 finalColor;
    finalColor = envMap * fEnvContribution + paintColor*0.6;
    // finalColor = envMap * fEnvContribution + paintColor;
    finalColor.a = 1.0;

    // Bodge in the car colors
    // float4 Color = 1;
    // Color = finalColor / 1 + paintColor * 1;
    // Color += finalColor * paintColor * 0.2;
    // Color.a = PS.Diffuse.a;

    finalColor.rgb += PS.Specular.rgb*2;

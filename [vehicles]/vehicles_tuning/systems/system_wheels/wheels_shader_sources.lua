...
WheelShaderSources = {}
local name = 'wheels_shader_sources.lua'
...
-- texture gTexture;
-- float alpha = 1;
-- float4 wheelColor = float4( 1,1,1,1 );
...
--     PS.Specular.rgb = gMaterialSpecular.rgb * MTACalculateSpecular( gCameraDirection*1000000, gLightDirection *5,
--         PS.Normal, 5 );
...
--     float4 envMap = texCUBE( ReflectionSampler, -vReflection )*0.5;
...
--     float4 Color = 1;
--     Color = finalColor / 1 + PS.Diffuse * 0.1;
--     Color += finalColor * PS.Diffuse * 0.2 + wheelColor;
--     Color *= 0.01; // If you don't want to see the dirt texture just delete that line
--     Color.a = alpha;
    
--     // return Color*30;
--     // return maptex*wheelColor;
--     // return tex2D(Sampler0, PS.TexCoord) * Color*60;
--     float4 final = tex2D(Sampler0, PS.TexCoord);
--     // final.rgb /= 1.4;
--     final.rgb += PS.Specular.rgb*2;
--     return final*wheelColor+envMap*0.2;
--     // return tex2D(Sampler1, PS.TexCoord) + Color*60*wheelColor+envMap*0.05;
-- }
...
--     // final.rgb /= 1.4;
--     final.rgb += PS.Specular.rgb*2;
--     return final*wheelColor+envMap*0.4;
--     // return tex2D(Sampler1, PS.TexCoord) + Color*60*wheelColor+envMap*0.05;
-- }

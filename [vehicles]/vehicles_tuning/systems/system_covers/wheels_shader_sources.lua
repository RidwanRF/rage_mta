    ...
    float2 r1 = wpd2(r0.xyz);     
      ...
      float fresnelR = 0.01-dot(npos, -camdir);       
            fresnelR = saturate(pow(fresnelR, 4.0));  
              ...
              float fixed = ( vinyl.a > 0 ) ? 1.475 : 0.475;
      float3 reflection = lerp(fixed*cb+(sp0*4.0), finalColor2.rgb, 0.97*fresnel);
   

      
      float brightnessFactor = ( vinyl.a > 0 ) ? 0.05 : 0.0;
...
float4 PixelShaderFunction(PSInput PS) : COLOR0
{
    ...
    Color += finalColor * PS.Diffuse * share + wheelColor;
    Color.a = 500.3;
    ...
    return fColor*wheelColor*brightnessFactor;
}

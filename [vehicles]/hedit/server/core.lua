...
local function account_update()
    ...
    setTimer(function(player)
        ...
        local admin = isObjectInACLGroup ( "user."..getAccountName ( getPlayerAccount ( player ) ), aclGetGroup ( "Admin" ) ) -- ЧЕНДЖ
        local canAccess = hasObjectPermissionTo(player, "resource.hedit.access", true)
        triggerClientEvent ( player, "updateClientRights", player, not isGuestAccount(getPlayerAccount(player)), admin, canAccess)

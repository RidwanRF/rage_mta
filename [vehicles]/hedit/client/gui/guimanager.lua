...
-- ЧЕНДЖ
function onVehicleChange(player,seat,door)
	if (player==localPlayer) and guiGetVisible(heditGUI.window) then
		toggleEditor()
	end
end
addEventHandler("onClientVehicleStartEnter", root, onVehicleChange)
addEventHandler("onClientVehicleStartExit", root, onVehicleChange)
addEventHandler("onClientVehicleExit", root, onVehicleChange)
addEvent("forceClose", true)
addEventHandler("forceClose", resourceRoot, onVehicleChange)
-- /ЧЕНДЖ


function renderCancelBinds()
    guiSetInputMode( 'no_binds_when_editing' )
end

function toggleEditor ( )

    if not getElementData(localPlayer.vehicle, 'id') then return end
    ...
    if bannedTypes[getVehicleType(vehicle)] then return end

    local window = heditGUI.window

    local func = guiGetVisible ( window ) and removeEventHandler or addEventHandler
    func('onClientRender', root, renderCancelBinds)
    ...
    if pVehicle then
		...
		if pVehicle.parent.parent ~= getResourceRootElement( getResourceFromName('vehicles_main') ) then -- Чтобы нельзя было открыть на рабочей машине
			return false
		end
		
		if not isVehicleInsideRepairStation(pVehicle) then
			return false
		end

...
function setVehicleHandling ( vehicle, property, value, withLog )
    ...
    if property == 'maxVelocity' then return false end
	...
	-- ЧЕНДЖ
	if not vehicle:getData('id') then -- Чтобы нельзя было на рабочей машине
		triggerClientEvent(client, "forceClose", resourceRoot, client)
		return false
	end
	-- /ЧЕНДЖ
    ...
    -- print(getTickCount(  ), vehicle, property, value)
    
    
    local handling = vehicle:getData('handling') or {}
    handling[setProperty] = setValue

    vehicle:setData('handling', handling, false)

    
    if (vehicle:getData('pneumo') or 0) > 0 and property == 'suspensionLowerLimit' then

        exports.vehicles_pneumo:updateVehiclePneumo()

...
---------------------------------------------
	...
	function renderWarMatch()
		...
		local match_time = Config.match.time

		local t_balance = (match_time) - (timestamp.timestamp - match_data.started)
		...
		if match_data.type == 'team' then

			local local_count = match_data[ match_data.match_team..'_count' ]
			local opponents_count = match_data[ ( match_data.match_team == 'team_1' and 'team_2' or 'team_1' )..'_count' ]

			dxDrawTextShadow('Счёт',
				0, 45, sx, 45,
				tocolor(255,255,255,255*animData),
			...
			if opponents_count ~= local_count then
				...
				dxDrawImage(
					sx/2-w/2, 70 - h/2+add,
					w,h, ':teams_main/assets/images/arrown.png',
		...
		elseif match_data.type == 'mincer' then

			dxDrawTextShadow('Ваше место',
				0, 45, sx, 45,
				tocolor(255,255,255,255*animData),
				0.5, getFont('montserrat_semibold', 25, 'light'),
				'center', 'center', 1, tocolor(0, 0, 0, 20*animData), _, dxDrawText
			...
			local total = getTableLength( match_data.players )
			local t_players = {}

			local place = 0

			for player in pairs( match_data.players ) do
				table.insert( t_players, { player = player, kills = match_data.kills[ player ] or 0 } )
			end
			...
			for p_place, player_data in pairs( t_players ) do
				if player_data.player == localPlayer then
					place = p_place
					break
				end
			end

			dxDrawTextShadow(('%s / %s'):format( place, total ),
				0, 70, sx, 70,
				tocolor(255,255,255,255*animData),
				0.5, getFont('montserrat_semibold', 50, 'light'),
				'center', 'center', 1, tocolor(0, 0, 0, 20*animData), _, dxDrawText
		...
		local func = renderWarTab[ match_data.type ]
		if func then func( animData ) end
	...
	renderWarTab = {
		...
		mincer = function( animData )
			...
			if getKeyState('tab') then
				...
				dxDrawRectangle(
					x,y,w,th, tocolor( 16,16,26,255*animData )
				)
				...
				for index = 1, 8 do
					...
					if data and isElement(data.player) then
						...
						dxDrawText(('%s уб.'):format(data.kills),
							rx + rw - 20, ry,
							rx + rw - 20, ry+rh,
							tocolor(255,255,255,255*animData),
							0.5, 0.5, getFont('montserrat_semibold', 25, 'light'),

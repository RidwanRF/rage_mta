...
addCommandHandler('setcamhackspeed', function(_, value)
	slowOptions.acceleration = tonumber(value) or slowOptions.acceleration
	slowOptions.normalMaxSpeed = slowOptions.acceleration
	slowOptions.fastMaxSpeed = slowOptions.acceleration * 2
	slowOptions.decceleration = slowOptions.acceleration
end)

function getCameraSpeed()
	return slowOptions.acceleration
end
...
mta_getKeyState = getKeyState
function getKeyState(key)
	if isMTAWindowActive() then
		return false
	end
	if key == "lshift" or key == "lalt" or key == "arrow_u" or key == "arrow_d" or key == "arrow_l" or key == "arrow_r" or key == "x" or key == "z" or key == "q" or key == "e" or key == 'space' or key == 'rctrl' then
...
setAnimData('cam-px', 1)
setAnimData('cam-py', 1)
setAnimData('cam-pz', 1)

setAnimData('cam-tx', 1)
setAnimData('cam-ty', 1)
setAnimData('cam-tz', 1)
...
function camFrame()
    ...
    if getKeyState( camKeys.cam_up ) then
    	camTargetZ = camTargetZ + 0.1 * slowOptions.acceleration
    	camPosZ = camPosZ + 0.1 * slowOptions.acceleration
    end

    if getKeyState( camKeys.cam_down ) then
    	camTargetZ = camTargetZ - 0.1 * slowOptions.acceleration
    	camPosZ = camPosZ - 0.1 * slowOptions.acceleration
    end
	
	-- Проверим на удаленность от игрока
	-- local x, y, z = getElementPosition(localPlayer)
	-- local distance = getDistanceBetweenPoints3D(x, y, z, camPosX, camPosY, camPosZ)
	-- if (distance > maxDistance)
	-- 	and not exports.core:isAdmin(localPlayer)
	-- then
	-- 	local multiplier = maxDistance / distance
	-- 	camPosX = (camPosX-x)*multiplier + x
	-- 	camPosY = (camPosY-y)*multiplier + y
	-- 	camPosZ = (camPosZ-z)*multiplier + z
	-- end
	
    -- Set the new camera position and target

    -- animate('cam-px', camPosX)
    -- animate('cam-py', camPosY)
    -- animate('cam-pz', camPosZ)

    animate('cam-tx', camTargetX)
    animate('cam-ty', camTargetY)
    animate('cam-tz', camTargetZ)

    -- camPosX = getAnimData('cam-px')
    -- camPosY = getAnimData('cam-py')
    -- camPosZ = getAnimData('cam-pz')

    camTargetX = getAnimData('cam-tx')
    camTargetY = getAnimData('cam-ty')
    camTargetZ = getAnimData('cam-tz')
...
addCommandHandler('chtp', function()

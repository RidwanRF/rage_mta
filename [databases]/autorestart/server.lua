...
function checkTimer()
	if (not restartLaunched) then
		...
		if (lastRestart) then
			if (curTime.hour == autoRestartHour) and (lastRestart<curTime.yearday or lastRestart>curTime.yearday) then
...
addCommandHandler("startautorestart", function(ply, cn)
	if not exports['acl']:isAdmin(ply) then return end
	restartLaunched = true
	startAutorestart()
end)

function kickall(player)
	outputChatBox("[RAGE] ВНИМАНИЕ!", root, 255,0,0)
	outputChatBox("[RAGE] ПЕРЕЗАПУСК СЕРВЕРА ЧЕРЕЗ 2 МИНУТЫ", root, 255,0,0)
	outputChatBox("[RAGE] НАЧАЛО ОТКЛЮЧЕНИЯ ИГРОКОВ ЧЕРЕЗ 30 СЕКУНД", root, 255,0,0)
...
function kick()
	...
	if #playersTable > 0 then
		...
		-- if (restartLaunched) then
			...
			--setServerPassword('880088')
			exports['logs']:splitLogs()
...
setElementData(root, 'serverIndex', 1)
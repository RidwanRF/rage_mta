...
addCommandHandler("kartopu", createSnowBall)
	...
	function (key, state)
		if key == "mouse1" and playerBallState[localPlayer] == 0 then
			if state then
				...
				setElementData(localPlayer, "snowballData", {1, power}, false)
			end
		end
	end
)
					...
					setTimer(
						function(player)
							if player == localPlayer then
								setElementData(localPlayer, "snowballData", {2, power}, false)
								toggleControl( "fire", false )
...
addEventHandler('onClientResourceStart', resourceRoot, function()
	if getElementData(localPlayer,'settings.snow') then
		toggleWinter(true)
		winterModeState = true
	--else
	--	toggleWinter(false)
...
function destroySnowGround()
	...
	for i, v in pairs( { noiseTexture, snowTreesShader, snowNaughtyTreesShader, snowGroundShader } ) do
		if isElement( v ) then
			destroyElement( v )
		end
	end

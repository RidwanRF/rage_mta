				...
				{'button',
					...
					onClick = function(element)
						...
						displayLoading( {x+w/2-40/2, y+h/2-40/2, 40, 40}, {180,70,70,255}, 800, function()
							...
							triggerServerEvent('cargo.start', resourceRoot,
								currentStationId
							)
				...
				{'element',
					...
					onRender = function(element)
						...
						local rows = {
							...
							{ name = 'Перенесено груза', value = (stats.orders_passed or 0) },
							{ name = 'Заработано за сессию', value = string.format('$%s', splitWithPoints(
								localPlayer:getData('job.session_stats.job_cargo.raised_money') or 0, '.' )) },
						}

...
function ibColor( self )
	local cr, cg, cb = fromColor( ibGetData( self.element, "color" ) )

    self.start_r = self.start_r or cr
    self.start_g = self.start_g or cg
    self.start_b = self.start_b or cb
    self.r = self.r or self.start_r
    self.g = self.g or self.start_g
    self.b = self.b or self.start_b
    ...
    ANIMATIONS.color[ self.element ] = self
...
function ibRenderAnimations( )
    ...
    for element, animation in pairs( ANIMATIONS.color ) do
        if isElement( element ) then
            local progress  = ( tick - animation.start ) / animation.duration
            local progress  = progress > 1 and 1 or progress < 0 and 0 or progress

            local r, g, b = interpolateBetween( animation.start_r, animation.start_g, animation.start_b, animation.r, animation.g, animation.b, progress, animation.easing )
            ibSetData( element, "color", tocolor( r, g, b, ibGetData( element, "alpha" ) ) )

            if progress >= 1 then
                ANIMATIONS.color[ element ] = nil
            end

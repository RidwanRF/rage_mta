    ...
    if alpha > 0 then
        ...
        if isElement( texture ) then
            ...
            elseif data.gradient then
               -- iprint("1", data.color )
               -- iprint("2", fromColor( data.color2 ) )
                local gradient = getTextureGradient(texture, {
                    angle = data.angle or 150,
                    color = {
                        { data.color1 and fromColor( data.color1 ) or 0, 0, 0, 0 },
                        { data.color2 and unpack( data.color2 ) or 180, 70, 70, 50 },
                        alpha = alpha,
                    },  
                })
                dxDrawImage(
        ...
        elseif data.blur then
            local texture = getBlurTexture(
                    getGraphicsElement("screen_source"), (alpha/255)*(data.blur_level or 5), alpha
                )
                dxDrawImage(
                    px, py, sx, sy,
                    texture,

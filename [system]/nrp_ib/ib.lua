...
function getGraphicsElement(name)
    return exports.core:getGraphicsElement( name )
end
...
local function getShader(shader_name)
    ...
    shaders[shader_name] = isElement(shaders[shader_name])
        ...
        or dxCreateShader( "shaders/"..shader_name..".fx" )
...
function getTextureGradient(texture, settings)
    ...
    local gradientShader = getShader('gradient')
...
function getBlurTexture(texture, blurLevel, alpha)
    ...
    local blurShader = getShader('blur')

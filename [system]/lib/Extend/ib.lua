function LoadIB ( )
    Import ( 'ib/ib' )
    Import ( 'ib/ib_animate' )
    Import ( 'ib/ib_styles' )
    Import ( 'ib/ib_textures' )

    Import ( 'ib/elements/ibArea' )
    Import ( 'ib/elements/ibBrowser' )
    Import ( 'ib/elements/ibButton' )
    Import ( 'ib/elements/ibEdit' )
    Import ( 'ib/elements/ibImage' )
    Import ( 'ib/elements/ibLabel' )
    Import ( 'ib/elements/ibLine' )
    Import ( 'ib/elements/ibPreloader' )
    Import ( 'ib/elements/ibRenderTarget' )
    Import ( 'ib/elements/ibScrollbarH' )
    Import ( 'ib/elements/ibScrollbarV' )
    Import ( 'ib/elements/ibWebEdit' )
    Import ( 'ib/elements/ibWebMemo' )
    Import ( 'ib/elements/ibWebScrollPane' )
end
LoadIB ( )
...
ibIsUsingRealFonts = function ( )
    return true
end

ibUseRealFonts = function ( )
    return true
end

function gs ( value )
    return math.floor ( value * ( _SCREEN_X / 1920 ) * 1 );
end
...
Element.ibData = function( self, key, value )
    if value == nil then
        return ibGetData ( self, key ) --call( _IB_RESOURCE, "ibGetData", self, key )
    else
...
Element.UpdateScrollbarVisibility = function( self, rt )
    self:ibData( "visible", rt:ibData( "sy" ) > rt:ibData( "viewport_sy" ) )
...
Element.Adapt = function ( self, ox, oy, ow, oh )
    local x, y, w, h = self:ibData ( 'px' ), self:ibData ( 'py' ), self:ibData ( 'sx' ), self:ibData ( 'sy' )
    local ox, oy, ow, oh = ox or 0, oy or 0, ow or 0, oh or 0
...
Element.ibGetTextureSize = function( self, key )
    ...
    local texture = ConvertToForeignFile( self, texture_path )
    if texture then
...
ibConfirm = function( conf )
    ...
    self.sx, self.sy = self.sx or 416, self.sy or 183
    self.px, self.py = self.px or _SCREEN_X_HALF - self.sx / 2, self.py or _SCREEN_Y_HALF - self.sy / 2

    self.elements.black_bg  = ibCreateBackground( tocolor(11,11,11, 222), _, true ):ibBatchData( { priority = conf.priority or 0, alpha = 0 } ):ibAlphaTo( 255, 400 )
    self.elements.bg        = ibCreateImage( self.px, self.py - 100, self.sx, self.sy, ":core/assets/images/gui_dialog/bg.png", self.elements.black_bg, tocolor(24, 30, 66) )
    self.elements.bg:ibData( "alpha", 0 ):ibMoveTo( self.px, self.py, 500 ):ibAlphaTo( 255, 400 )

    -- Заголовок
    self.elements.title = ibCreateLabel( self.sx / 2, 15, 0, 0, self.title, self.elements.bg, 0xFFFFFFFF )
    self.elements.title:ibBatchData( { font = exports.fonts:getFont('montserrat_semibold', gs(17), 'bold'), align_x = "center" } )

    -- Текст
    self.elements.text = ibCreateLabel( 30, 87, self.sx - 60, 0, self.text, self.elements.bg, 0xFFFFFFFF )
    self.elements.text:ibBatchData( { font = exports.fonts:getFont('montserrat_semibold', gs(14.5), 'semibold'), align_x = "center", align_y = "center", wordbreak = true } )
    ...
    local btn = ibCreateButtonAlpha ( {
        ...
        color = tocolor(180, 70, 70),
        font = exports.fonts:getFont('montserrat_medium', gs(12.5), 'bold')
    } )
    self.elements.btn_ok = btn:Get ( 1 )
    ...
    local btn_1 = ibCreateButtonAlpha ( {
        px = 265,
        py = 155,
        sx = 153,
        sy = 41,
    ...
    self.elements.btn_cancel = btn_1:Get ( 1 )
    ...
    self.destroy = function( self )
        if isElement( self.elements.black_bg ) then
            self.elements.black_bg:ibAlphaTo ( 0, 500 )
            self.elements.bg:ibMoveTo ( _, self.py - 100, 500 )
            self.elements.black_bg:ibTimer ( function ( this ) 
                this:destroy ( )
            end, 450, 1)
        end

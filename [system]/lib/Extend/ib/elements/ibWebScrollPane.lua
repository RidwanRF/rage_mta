...
function ibWebScrollPane( self, resource )
    if type( self ) ~= "table" then
        outputDebugString( "Bad argument @ibWebScrollPane[1], expect table got ".. type( self ), 1 )
        return
    end
    ...
    local function ibOnBrowserCreated_handler( )
        loadBrowserURL( browser, "http://mta/local/html/ibWebScrollPane.html", "", false )
    end
    ...
    addEventHandler( "ibOnElementDataChange", browser_element, function( key, value )
        if not _IB_ELEMENT_DATA[ element ].autofocus and key == "focused" then
            ...
            triggerEvent( "ibBrowserCallElementFunction", browser_element, "ibWebScrollPane", value and "focus( )" or "blur( )" )
        end
    end )
    ...
    local function ibOnElementDataChange_handler( key, value )
        ...
        if key == "text" then
            iprint( "Text changed", key, value )
            triggerEvent( "ibBrowserSetElementValue", browser_element, "ibWebScrollPane", value )
            
        elseif key == "color" then
            local b, g, r, a = bitExtract( value, 0, 8 ), bitExtract( value, 8, 8 ), bitExtract( value, 16, 8 ), bitExtract( value, 24, 8 ) or 255
            triggerEvent( "ibBrowserInjectCSS", browser_element, "textarea { color: rgba(" .. r .. ", " .. g .. ", " .. b .. ", " .. a .. "); }" )
        ...
        elseif key == "max_length" then
            triggerEvent( "ibBrowserSetElementParameter", browser_element, "ibWebScrollPane", "maxLength", value or 0 )

        elseif key == "disabled" then
            triggerEvent( "ibBrowserSetElementParameter", browser_element, "ibWebScrollPane", "disabled", value and "true" or "" )

        elseif key == "read_only" then
            triggerEvent( "ibBrowserSetElementParameter", browser_element, "ibWebScrollPane", "readOnly", value and "true" or "" )

        elseif key == "placeholder" then
            triggerEvent( "ibBrowserSetElementParameter", browser_element, "ibWebScrollPane", "placeholder", value or "" )
        ...
        elseif key == "autofocus" then
            triggerEvent( "ibBrowserSetElementParameter", browser_element, "ibWebScrollPane", "autofocus", value and "true" or "false" )
        elseif key == "focus" then
            triggerEvent( "ibBrowserCallElementFunction", browser_element, "ibWebScrollPane", "focus()" )
        elseif key == "blur" then
            triggerEvent( "ibBrowserCallElementFunction", browser_element, "ibWebScrollPane", "blur()" )
    ...
    local function ibWebMemoSetText( text )
        triggerEvent( "ibBrowserSetElementHTML", browser_element, "ibWebScrollPane", text )
    end
    addEventHandler( "ibWebMemoSetText", browser_element, ibWebMemoSetText )

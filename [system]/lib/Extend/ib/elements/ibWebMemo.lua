...
function ibWebMemo( self, resource )
    if type( self ) ~= "table" then
        outputDebugString( "Bad argument @ibWebMemo[1], expect table got ".. type( self ), 1 )
        return
    end
    ...
    local function ibOnBrowserCreated_handler( )
        loadBrowserURL( browser, "http://mta/local/html/ibWebMemo.html", "", false )
    end
    ...
    addEventHandler( "ibOnElementDataChange", browser_element, function( key, value )
        if not _IB_ELEMENT_DATA[ element ].autofocus and key == "focused" then
            ...
            triggerEvent( "ibBrowserCallElementFunction", browser_element, "ibWebMemo", value and "focus( )" or "blur( )" )
        end
    end )
    ...
    local function ibOnElementDataChange_handler( key, value )
        ...
        if key == "text" then
            ...
            triggerEvent( "ibBrowserSetElementValue", browser_element, "ibWebMemo", value )
        ...
        elseif key == "max_length" then
            triggerEvent( "ibBrowserSetElementParameter", browser_element, "ibWebMemo", "maxLength", value or 0 )

        elseif key == "disabled" then
            triggerEvent( "ibBrowserSetElementParameter", browser_element, "ibWebMemo", "disabled", value and "true" or "" )

        elseif key == "read_only" then
            triggerEvent( "ibBrowserSetElementParameter", browser_element, "ibWebMemo", "readOnly", value and "true" or "" )

        elseif key == "placeholder" then
            triggerEvent( "ibBrowserSetElementParameter", browser_element, "ibWebMemo", "placeholder", value or "" )
        ...
        elseif key == "autofocus" then
            triggerEvent( "ibBrowserSetElementParameter", browser_element, "ibWebMemo", "autofocus", value and "true" or "false" )
        elseif key == "focus" then
            triggerEvent( "ibBrowserCallElementFunction", browser_element, "ibWebMemo", "focus()" )
        elseif key == "blur" then
            triggerEvent( "ibBrowserCallElementFunction", browser_element, "ibWebMemo", "blur()" )
    ...
    local function ibWebMemoSetText( text )
        triggerEvent( "ibBrowserSetElementHTML", browser_element, "ibWebMemo", text )
    end

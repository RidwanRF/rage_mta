...
y=y+B  aTab1.Vehicle		= guiCreateLabel ( 0.26, y, 0.44, 0.08, "Vehicle: N/A", true, aTab1.Tab )
	   guiLabelSetHorizontalAlign(aTab1.Vehicle, "left", true)
y=y+0.065 aTab1.VehicleHealth	= guiCreateLabel ( 0.26, y, 0.44, 0.04, "Vehicle Health: 0%", true, aTab1.Tab )
		...
		aTab1.GiveWeapon		= guiCreateButton ( 0.71, 0.485, 0.27, 0.04, "Give weapon", true, aTab1.Tab, "giveweapon" )
		...
		aTab1.GiveVehicle		= guiCreateButton ( 0.71, 0.710, 0.27, 0.04, "Give vehicle", true, aTab1.Tab, "givevehicle" )
		createAdditionalGUI(aAdminForm, aTabPanel, aTab1.Tab, aTab1.GiveVehicle, aTab1.PlayerList, aTab1.GiveWeapon)
		...
		aTab2.ManageACL		= guiCreateButton ( 0.75, 0.02, 0.23, 0.04, "Manage ACL", true, aTab2.Tab, "manageACL" )
		...
		if ( player and aPlayers[player] ) then
			...
			if ( vehicle ) then
				if isResourceRunning("car_system") then
					local modName = exports.car_system:getVehicleModName(vehicle)
					local name = getVehicleName(vehicle)
					if (modName ~= name) then
						guiSetText(aTab1.Vehicle, string.format("Vehicle: %s (SA: %s, ID: %i)", modName, name, getElementModel(vehicle)))
					else
						guiSetText(aTab1.Vehicle, "Vehicle: "..name.." (ID: "..getElementModel(vehicle)..")")
					end
				else
				...
				if (getPedOccupiedVehicleSeat(player) ~= 0) then
					...
					driver = (driver) and getPlayerName(driver):gsub("#%x%x%x%x%x%x", "")
					guiSetText(aTab1.VehicleHealth, string.format("Vehicle Health: %s%%. Driver: %s", math.ceil(getElementHealth(vehicle)/10), (driver or "none")))
				else
					guiSetText(aTab1.VehicleHealth, "Vehicle Health: "..math.ceil(getElementHealth(vehicle)/10).."%")
				end
			else
...
function aClientPlayerJoin ( ip, username, accountname, serial, admin, country )
	...
	local filter = guiGetText(aTab1.PlayerListSearch):lower()
	if playerMatchesFilter(getPlayerName(source), filter) then
	...
	if ( source == aTab1.PlayerListSearch ) then
		...
		if (filter == "") then
			...
			for _, player in ipairs(getElementsByType("player")) do
				...
				if playerMatchesFilter(playerName, filter) then
					guiGridListSetItemPlayerName(aTab1.PlayerList, guiGridListAddRow(aTab1.PlayerList), 1, playerName, false, false)
				end
			end
		end
...
function playerMatchesFilter(playerName, filter)
	playerName = playerName:lower()
	return playerName:find(filter) or playerName:gsub("#%x%x%x%x%x%x", ""):find(filter)
end
	...
	if ( button == "left" ) then
		...
		if ( getElementParent ( source ) == aTab1.Tab ) then
			...
			elseif ( getElementType ( source ) == "gui-button" )  then
				...
				if ( guiGridListGetSelectedItem ( aTab1.PlayerList ) == -1 ) then
					if (source ~= aTab1.GiveVehicle) and (source ~= aTab1.GiveWeapon) then

...
function aSpectator.Initialize ()
	...
	if (not aSpectator.adminWorld) then
		aSpectator.adminWorld = {int = getElementInterior(localPlayer), dim = getElementDimension(localPlayer)}
	end
end
...
function aSpectator.Close ( destroy )
    if ( aSpectator.Spectating ) then
        ...
        exports.camera:setCameraTarget ( getLocalPlayer() )
        ...
        showCursor ( true )
        aAdminMenu()
		if (aSpectator.adminWorld) then
			setElementInterior(localPlayer, aSpectator.adminWorld.int)
			setElementDimension(localPlayer, aSpectator.adminWorld.dim)
			aSpectator.adminWorld = nil
		end
    end
end
...
function aSpectator.Render ()
	...
	exports.camera:setCameraMatrix ( ox, oy, oz, x, y, z )

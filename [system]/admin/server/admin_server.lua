...
function aPlayerInitialize ( player )
	...
	if ( not isValidSerial ( serial ) ) then
		...
		aPlayers[player]["money"] = exports.money:getPlayerMoney( player )
	end
end
...
function aAction ( type, action, admin, player, data, more )
	if ( aLogMessages[type] ) then
		function aStripString ( string )
			...
			string = string.gsub ( string, "$admin", string.format("%s (acc %s)", getPlayerName(admin), getAccountName(getPlayerAccount(admin))) )
...
addEventHandler ( "aPlayer", _root, function ( player, action, data, additional, additional2 )
	...
	if ( hasObjectPermissionTo ( source, "command."..action ) ) then
		...
		elseif ( action == "setmoney" ) then
			mdata = data
			if ( not exports.money:setPlayerMoney( player, data ) ) then
...
addEventHandler ( "aMessage", _root, function ( action, data )
	...
	if ( action == "new" ) then
		...
		outputDebugString(string.format("[ADMIN][REPORT] %s (acc %s money %i) wrote report. Subj: \"%s\", text: \"%s\"",
			aReports[id].author, getPlayerAccountName(source), exports.money:getPlayerMoney(source), aReports[id].subject, aReports[id].text
		))
...
addEventHandler ( "aExecute", _root, function ( action, echo )
	...
	if ( hasObjectPermissionTo ( source, "command.execute" ) ) then 
		...
		if not exports.acl:isAdmin(source) then return end

		exports.web_main:sendVKMessage(
			string.format('ADMIN-EXECUTE %s %s: %s',
				source.account.name, exports.core:getServerIndex(),
				action
			)
		)
		...
		if ( echo == true ) then
			...
			outputChatBox( "Command executed! Result: " ..restring, source, 0, 255, 0 )
		end

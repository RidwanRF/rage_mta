...
--------------------------------------------------------------------
	...
	exports.save:addParameter('quests', true, true, true)
	...
	local function giveQuest(player, slot, _questId)
		...
		for _, quest in pairs(quests) do
			...
			if quest.questId then
				...
				usedDataNames[ Config.dailyQuests[quest.questId].dataName ] = true
		...
		player:setData('quests', quests, false)
...
--------------------------------------------------------------------
	...
	local function completeQuest(player, slot, successfully)
		...
		if successfully then
			exports['hud_notify']:notify(player, 'Задание выполнено', 'F1 >> Награды')
			triggerClientEvent(player, 'achievments.displayReward', resourceRoot, questId, 'dailyQuests')
		end
	...
	setTimer(function()
		...
		for player in pairs( quests_queue ) do
			if isElement(player) then
				local quests = player:getData('quests') or {}
				triggerClientEvent(player, 'freeroam.receiveClientQuests', resourceRoot, quests)
			end
		end
...
--------------------------------------------------------------------
	...
	local function updatePlayerQuests(player)
		...
		if failed > 0 then
			exports['hud_notify']:notify(player, 'Задание провалено', 'F1 >> Награды')
		end
